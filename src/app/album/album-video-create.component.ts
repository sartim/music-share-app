...
@Component({
    templateUrl: 'album-video-create.component.html',
    styles: []
})

export class AlbumVideoCreateComponent implements OnInit {
    model: any = {};
    loading = false;
    currentUser: User;

    @ViewChild('video_file') video_file;
    ...
    submit_() {
      ...
      let uploadFile = (<HTMLInputElement>window.document.getElementById('video_file')).files[0];

      let myUploadItem = new MyUploadItem(uploadFile, 'https://promoh.herokuapp.com/api/v1/video/');
        myUploadItem.formData = {
          album_id: arr[0],
          video_file: myUploadItem.file,
          song_title: this.model.album_title

        };
        this.uploaderService.onSuccessUpload = (item, response, status, headers) => {
          this.presentToast('Video added successfully');
